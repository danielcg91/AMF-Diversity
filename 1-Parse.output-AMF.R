...
#Stats by sample
data=readLines("~/Google-Drive/stats/trimmed.out.txt")
out=data.frame(aim.id=gsub(".fastq.gz","",gsub("~/Google-Drive/trimmed/","",grep("fastq.gz",data, value = T))),
...
data=readLines("~/Google-Drive/stats/merged.out.txt")
sub=data.frame(sample=str_extract(grep("extendedFrags",data, value = T),"\\d+\\D+\\d+.....\\d+"),
...
data=readLines("~/Google-Drive/stats/filter.out.txt")
kept.pairs=str_extract(grep("truncated", data, value = T), "^\\d+")
sub=data.frame(sample=str_extract(grep("NGSeco",data, value = T),"20210219\\D+\\d+.....\\d+"),
...
data=readLines("~/Google-Drive/stats/derep.sample.out.txt")
...
write.table(out,file = "~/Google-Drive/stats/processing.stats.txt", row.names = F, col.names = T, quote = F,
            sep = "\t")

#stats of otus

data=readLines("~/Google-Drive/stats/derep.all.out.txt")
...
data=readLines("~/Google-Drive/stats/otu.out.txt")
...
data=readLines("~/Google-Drive/stats/table.out.txt")
...
write.table(out.con,file = "~/Google-Drive/stats/clustering.stats.txt", row.names = T, col.names = F, quote = F,
            sep = "\t")
...
sintax=read.delim(file=paste("~/Google-Drive/result/OTUs_semiraw","otus.sintax",sep = "/"), header = F, stringsAsFactors = F)
...
taxa=t(taxa)
write.table(taxa,file = "~/Google-Drive/stats/classification.stats.txt", row.names = T, col.names = F, quote = F,
            sep = "\t")
