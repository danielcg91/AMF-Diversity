...
if ( amplicon == "16s"){
  setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/agave16s/16S")
  taxa=c(rdp="otus.sintax.rdp",silva="otus.sintax.silva",green="otus.sintax.green")[2]
  ...
  setwd("~/Documentos/agaveITS/ResultsITS_sr") 
...
rownames(sintax)=sintax$otu.id ; sintax=sintax[,c(2:7,1)]
  ...
  # Load METADATA 
meta=read.delim(paste("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/agave16s/16S",metadata,sep = "/"))
...
# Select the files
table=c("otutab.txt")[1]
metadata=c("vicdani_metadata.txt", "vicdani_metadatagood.txt")[1] #this can be a different name but must be the complete 309 sample metadata
fasta=c("otus.fasta")[1]
...
# Load METADATA 
meta=read.delim(paste("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/agaveITS/ITS2_1/",metadata,sep = "/"))
meta$AIM.sample.id=gsub("-","_",meta$AIM.sample.id)
rownames(meta)=meta$AIM.sample.id
meta=meta[order(meta$AIM.sample.id),] #order samples
meta$plate=c(rep("E116",80),rep("E117",80),rep("E118",80),rep("E119",69)) #add plate information 
...
otu.a=otu[!rownames(otu) %in% unique(c(n,e,c,a,amo,eu,euk,mtz,pro,stra,rhi)),grep("neg",colnames(otu), invert = T)] 
...
#OBTAINF OTUS DE AMF
library(Biostrings)
AMF = readDNAStringSet("otus.daniel.fasta")
