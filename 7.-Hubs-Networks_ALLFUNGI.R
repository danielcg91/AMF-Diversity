...

{
  ...
  FUNGI=phyloseq(otu_table(otu, taxa_are_rows = T),tax_table(taxon),sample_data(met))
  save(FUNGI, file = paste(getwd(),paste(t,exit_name,"phyloseq",sep="_"),sep = "/"))
...
load("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/network.paper2/spores_FUNGI_phyloseq")

FUNGI

tax_tbl <- as.data.frame(FUNGI@tax_table@.Data)

#Using spiec.easi

#using default parameter for spores
se_mb= spiec.easi(FUNGI, method='mb',lambda.min.ratio=1e-2, nlambda=100, 
...
saveRDS(se_mb,"allspofun_mb.RDS")
...
se_mb <- readRDS(file = "allspofun_mb.RDS")
...
se_net <- adj2igraph(getRefit(se_mb),
                     ...
                     vertex.attr = list(name = taxa_names(FUNGI)))
...
#a=plot_network(se_net, AMF, type = "taxa", color = "order", shape = "domain", label = NULL, title = t)
#print(a)
#dev.off()
