...
t=c("Agave.tequilana", "Agave.salmiana", "Myrtillocactus.geometrizans","spores")[4]
...
{
  ## Generate OTU table
  setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/network.paper2")
  data.dir=("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/network.paper2/")
  main.dir=getwd()
  ...
  pc=c("soil", "rhizosphere", "root endosphere", "spores")[4]
  #tr=c("T0", "T4", "T8", "T12")[4]
  ps=c("Agave.tequilana", "Agave.salmiana", "Myrtillocactus.geometrizans")[1:3]
  ...
  #}
  for ( t in pc){
  samples=rownames(met.2[met.2$plant.species %in% ps & met.2$plant.compartment %in% t,])
  }
  otus=otu[,samples]
  ...
  AMF=phyloseq(otu_table(otu, taxa_are_rows = T),tax_table(taxon),sample_data(met))
  save(AMF, file = paste(getwd(),paste(t,exit_name,"phyloseq",sep="_"),sep = "/"))
...
#load("~/Library/Mobile Documents/com~apple~CloudDocs/Documentos/network.paper2/spores_AMF_phyloseq")
...
se_mb= spiec.easi(AMF, method='mb',lambda.min.ratio=1e-1, nlambda=100, 
                    pulsar.params=pargs, ncores=6)

#using default parameter for A. salmiana
se_mb= spiec.easi(AMF, method='mb',lambda.min.ratio=1e-2, nlambda=100, 
...
se_net <- adj2igraph(getRefit(se_mb), 
                     ...
                     vertex.attr = list(name = taxa_names(AMF))) # Usamos el ID de las taxas para nombrar los vértices o nodos de la red
#save igraph
...
a=plot_network(se_net, AMF, type = "taxa", color = "phylum", shape = "domain", label = NULL, title = t)
print(a)
dev.off()
...
tax_tbl <- as.data.frame(AMF@tax_table@.Data)
...
colorsphylum=c("p:Proteobacteria"="#6972D8", "p:Actinobacteria"="#bf360c","p:Tenericutes"="#007FD8", 
          "p:Bacteroidetes"= "#00837A","p:Acidobacteria"= "#F9F871","p:Gemmatimonadetes"="#FFCC5E",
          "p:Fusobacteria"= "#009688","p:Chloroflexi"="#FF9800","p:Verrucomicrobia"="#795548", 
          "p:Planctomycetes"= "#91003f","p:Saccharibacteria" = "#4a148c","p:Deinococcus-Thermus"="#845A12",
          "p:Gemmatimonadetes"="#03A9F4", "p:Hydrogenedentes"="#9C27B0","p:Glomeromycota"= "#10441A","p:Unidentified" = "grey20")
#"Marinisomatota"= "#00BCD4",, #"p:Gemmatimonadetes"="#2196F3", #"Myxococcota" = "#FFEB3B", 
#"Nanoarchaeota"= "#607D8B",#"Nitrospinota"= "#F44336", #"Nitrospirota"= "#c7e9b4", #"Patescibacteria" = "#4CAF50", 
