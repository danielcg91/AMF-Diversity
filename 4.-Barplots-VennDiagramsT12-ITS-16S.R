...
if (amplicon == "ITS2"){
  ...
  #order
  low = orden[orden$relab < 2.098661e-01, "rank"]
  #phylum
  ...
  otu=otu[rownames(met)]
  ####Total reads by PLANT SPECIE AND PLANT COMPARTMENT
  t=c("domain","phylum","class","order","family","genus","otu.id")[7]
  ##Sumarise data
  set.seed(23171341)
  otu.s=t(rrarefy(t(otu), sample = min(colSums(otu))))
  #Checklist
...
p1=ggplot(data=otu.t1, aes(y=relab, x=plant.species, fill=rank))+
  geom_bar(stat="identity",width = 0.95)+
  scale_fill_manual(values = my_pal(n))+
  labs(x = "Plant species",y = "relative abundance (%)")+
  facet_grid(~plant.compartment, scales = "free", space = "free_x")+tema+
