...
if (amplicon == "AMF"){
  ...
  colSums(otu.s)
  #Subset AMF
  tax= subset(tax, tax$phylum == "p:Glomeromycota", select= c("domain","phylum","class","order","family","genus","otu.id"))
  ...
  shan=diversity(t(otu.s),index = "shannon")
    ...
    colSums(otu.s)
    #Subset 16-S
    tax= tax[rownames(taxeb),]
    otu=otu[rownames(tax),]
    #subset only 16-S otus
    otu.s=as.data.frame(otu.s[rownames(tax),])
    #Calculate alpha-diversity
    ric=(rarefy(t(otu), sample = min(colSums(otu))))
    #ric=(rarefy(t(otu), sample = 931))
...
mean.r=summarySE(data = alpha, measurevar = "richness", groupvars = "plant.species")
mean.r$plant.species=factor(mean.r$plant.species, level = pc)
mean.s=summarySE(data = alpha, measurevar = "shannon", groupvars = "plant.species")
mean.s$plant.species=factor(mean.s$plant.species, level = pc)

#RICHENESS BY PLANT.COMPARTMENT
pdf(paste(amplicon, file = "plant.species.richenessplot.pdf"), colormodel = "cmyk", width = 8.5, height =8.5, compress = F)
p=ggplot(data=mean.r, aes(x=plant.species, y=richness))+
  geom_bar(stat="identity", fill=c("gray100","gray80","gray50", "gray30"), colour="black", size=1)+
  geom_errorbar(aes(ymin=richness-sd, ymax=richness+sd),width=0.2,position=position_dodge(1.5))+
  #geom_dotplot(data=alpha,binaxis='y', stackdir='center', stackratio=2, dotsize=0.8, aes(fill=treatment))+
  scale_fill_manual(values = my_pal(9)[4:8])+tema
...
dev.off()
#SHANNON BY PLANT.COMPARTMENT
pdf(paste(amplicon, file = "plant.species.shannonplot.pdf"), colormodel = "cmyk", width = 8.5, height = 8.5, compress = F)
p1=ggplot(data=mean.s, aes(x=plant.species, y=shannon))+
  geom_bar(stat="identity",fill=c("gray100","gray80","gray50", "gray30"),colour="black",size=1)+
  geom_errorbar(aes(ymin=shannon-sd, ymax=shannon+sd),width=0.2,position=position_dodge(1.5))+
  #geom_dotplot(data=alpha,binaxis='y', stackdir='center', stackratio=2, dotsize=0.8, aes(fill=treatment))+
  scale_fill_manual(values = my_pal(9)[1:4])+tema
...
#Plant species
pdf(paste(amplicon, file = "NMDS.plantspecies.pdf"), colormodel = "srgb", width = 11, height = 8.5, compress = F)
...
#plant.compartment
pdf(paste(amplicon, file = "NMDS.plantspecies-1.pdf"), colormodel = "srgb", width = 11, height = 8.5, compress = F)
m=ggplot(data=scaling3, aes(x=X1, y=X2, colour=plant.species, shape=plant.compartment))+
  geom_hline(yintercept = 0, linetype=2)+
  geom_vline(xintercept = 0,linetype=2)+
  geom_point(size=9)+
  #scale_colour_manual(values = my_pal(9)[2:5])+
...
######
pdf(paste(amplicon, file = "plantcompartment.distances.pdf"), colormodel = "cmyk", width = 8.5, height =8.5, compress = F)
...
##Plant.species
mod = with(met, betadisper(scaling, plant.species))
alpha$distance_to_centroid=mod$distances
mean.c=summarySE(data = alpha, measurevar = "distance_to_centroid", groupvars = "plant.species")
mean.c$plant.species=factor(mean.c$plant.species, level = pc)
######
pdf(paste(amplicon, file = "plant.species.distances.pdf"), colormodel = "cmyk", width = 8.5, height =8.5, compress = F)
d=ggplot(data=mean.c, aes(x=plant.species, y=distance_to_centroid, fill=plant.species))+
  geom_bar(stat="identity",colour="black",size=1, fill=c("gray100","gray80","gray50", "gray30"))+
  geom_errorbar(size=1,aes(ymin=distance_to_centroid-sd, ymax=distance_to_centroid+sd),width=0.2,position=position_dodge(1.5))+
  scale_fill_manual(values = my_pal(12)[c(1,4,7,9)])+
  scale_y_continuous(limits = c(0,0.6), breaks = seq(0,1,0.2))+tema
